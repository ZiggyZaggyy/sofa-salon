...
export function reservationsUpdateHint(hasServiceRole: boolean): string {
  ...
  return (
    'Could not update reservation attendance. Add SUPABASE_SERVICE_ROLE_KEY to .env.local ' +
    'or run supabase-sql/30-reservations-admin-update-rls.sql in Supabase.'
  );
}
